...
class LinearPredictionGuider(Guider):
    ...
    def __call__(self, x: torch.Tensor, sigma: torch.Tensor, num_video_frames=None) -> torch.Tensor:
        # modified: support of variable num_video_frames
        x_u, x_c = x.chunk(2)
        if num_video_frames is not None:
            num_frames = num_video_frames
            scale = torch.linspace(self.min_scale, self.max_scale, num_frames).unsqueeze(0)
        else:
