...
# modified: add video logger
class VideoLogger(ImageLogger):
    @rank_zero_only
    def log_local(self, save_dir, split, videos, fps, global_step, current_epoch, batch_idx):
        root = os.path.join(save_dir, "image_log", split)
        for k in videos:
            filename = "{}_gs-{:06}_e-{:06}_b-{:06}".format(k, global_step, current_epoch, batch_idx)
            path = os.path.join(root, filename)
            os.makedirs(os.path.split(path)[0], exist_ok=True)            
            export_to_video(videos[k], path + '.mp4', save_to_gif=False, use_cv2=False, fps=fps)
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and 
            ...
            fps = int(batch['fps_id'][0].item())
            self.log_local(pl_module.logger.save_dir, split, images, fps,
                           pl_module.global_step, pl_module.current_epoch, batch_idx)
...
class TextProgressBar(pl.callbacks.ProgressBarBase):
    ...
    def on_validation_start(self, trainer, pl_module):
        ...
        msg = f'[Validation begins] [Epoch {trainer.current_epoch}] [Training batch {self.train_batch_idx}] [Val batch num: {self.total_val_batches}] => '
        self._logger.info(msg)
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        if self._should_update(current, self.total_val_batches):
            ...
            msg = f'[Validation] [Epoch {trainer.current_epoch}] [Training batch {self.train_batch_idx}] [Val batch {self.val_batch_idx}/{self.total_val_batches} {batch_time:.2f} s/batch]  => '
            filter_fn = None
            msg += self._serialize_metrics(trainer.progress_bar_metrics, filter_fn=filter_fn)
            self._logger.info(msg)
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        msg = f'[Validation finished] [Epoch {trainer.current_epoch}] [Training batch {self.train_batch_idx}] => '
        filter_fn=lambda x: x.startswith('test') and x.endswith('_avg')
        msg += self._serialize_metrics(trainer.progress_bar_metrics, filter_fn=filter_fn)
        self._logger.info(msg)
    # modified: add testing hooks
    def on_test_start(self, trainer, pl_module):
        super().on_test_start(trainer, pl_module)
        self._logger.info(f'[Testing Begins] [Epoch {trainer.current_epoch}]')

    def on_test_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        super().on_test_batch_end(trainer, pl_module, outputs, batch, batch_idx, dataloader_idx)
        current = self.test_batch_idx
        if self._should_update(current, self.total_test_batches):
            batch_time = (time.time() - self._time) / self.test_batch_idx
            msg = f'[Testing] [Epoch {trainer.current_epoch}] [Val Batch {self.test_batch_idx}/{self.total_test_batches} {batch_time:.2f} s/batch] => '
            filter_fn = lambda x: x.startswith('test') and not x.endswith('_avg')
            # filter_fn = None
            msg += self._serialize_metrics(trainer.progress_bar_metrics, filter_fn=filter_fn)
            self._logger.info(msg)
    ...
    def on_test_end(self, trainer, pl_module):
        ...
        msg = f'[Testing finished] [Epoch {trainer.current_epoch}] => '
        filter_fn=lambda x: x.startswith('test') and x.endswith('_avg')
        # filter_fn = None
        msg += self._serialize_metrics(trainer.progress_bar_metrics, filter_fn=filter_fn)
        self._logger.info(msg)
