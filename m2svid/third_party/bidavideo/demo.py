...
def demo(args):
    ...
    with torch.no_grad():
        ...
        images_left = sorted(glob.glob(os.path.join(args.path, 'right/*.png')) + glob.glob(os.path.join(args.path, 'right/*.jpg')))
        images_right = sorted(glob.glob(os.path.join(args.path, 'left/*.png')) + glob.glob(os.path.join(args.path, 'left/*.jpg')))

        # images_left = sorted(glob.glob(os.path.join(args.path, 'image_left/*.png')) + glob.glob(os.path.join(args.path, 'image_left/*.jpg')))
        # images_right = sorted(glob.glob(os.path.join(args.path, 'image_right/*.png')) + glob.glob(os.path.join(args.path, 'image_right/*.jpg')))

        # images_left = images_left[:10]
        # images_right = images_right[:10]
        ...
        for start_idx in range(0, num_frames, frame_size):
            ...
            disparities_raw = predictions["disparity"].clone().data.cpu().numpy()
            ...
            disparities_raw_ori_all.extend(disparities_raw)

        disparities_ori_all = np.array(disparities_ori_all)
        disparities_raw_ori_all = np.array(disparities_raw_ori_all)
        np.save(os.path.join(args.output_path, "disparity"), disparities_ori_all)
        np.save(os.path.join(args.output_path, "disparity_raw"), disparities_raw_ori_all)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resize', default=(720, 1280), nargs='+', type=int, help="image size input to the model")
